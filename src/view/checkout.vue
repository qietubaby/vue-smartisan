...
<template>
	<div id="main">
						...
						<div class="box-inner js-checkout-address-panel ">
							<div class="address-common-table js-multiple-address-panel">
								<ul class="address-item-list clear js-address-item-list">
									<li @click="chooseReceive(index)" v-for="receive,index in receiveInfo" class="js-choose-address" :class="{'selected-address-item':receiveIndex==index}">
									...
									<li @click="showPop" class="add-address-item js-add-address">
										<p>使用新地址</p>
									</li>
								</ul>
							</div>
						...
						<div class="invoice-detail"> 发票抬头： 
							<div class="radio-box"> 
								<label @click="checkedInvoice(true)"> 
									...
									<span class="blue-radio" :class="{'blue-radio-on':invoice.prosonal}"><a></a></span>  个人
								</label> 
								<label @click="checkedInvoice(false)"> 
									<input type="radio" class="hide"> 
									<span :class="{'blue-radio-on':!invoice.prosonal}" class="blue-radio"><a></a></span>  单位
								</label> 
							</div> 
							<div v-if="!invoice.prosonal" class="module-form-row form-item fn-hide js-invoice-title"> 
			...
			<addressPop v-if="popShow" @close="closePop"></addressPop>
		</div>
</template>
<script>
